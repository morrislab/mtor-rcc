...
class MutEx(object):
    ...
    def calculate(self, indices: list, n=None, parallel=True, cores=0) -> MutExResult:
        ...
        simulated_results = pool.starmap(partial_simul.func,
                                         zip(itertools.repeat(coverage, n), itertools.repeat(observation_signal.astype(int), n)))
...
def main():
    ...
    parser.add_argument('fn', type=str, help='file with sample x mutation matrix')
    parser.add_argument('permutations', type = int, help = 'number of permutations')
    args = parser.parse_args()

    adj = pd.read_csv(args.fn, index_col = 0)
    m = MutEx(background=adj, permutations=args.permutations)
    groups = [['MTOR', 'PIK3CA'], ['MTOR', 'PTEN'], ['MTOR', 'PIK3CA', 'PTEN'], ['PIK3CA', 'PTEN']]
    result_list = [m.calculate(g) for g in groups]
    print(pd.DataFrame.from_records([r.__dict__ for r in result_list]))
