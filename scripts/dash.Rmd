...
Data retrieved from [GDC data portal](https://portal.gdc.cancer.gov/). Data displayed is from MC3 project, and aligned to hg19. MAF files an oncoplots were prepared using [maftools](https://www.bioconductor.org/packages/release/bioc/html/maftools.html). 
...
maf_fns <- list.files(maf_dir, pattern = "*.maf")[c(11,21)]
...
for (g in names(df)) {
  ...
  p <- p %>% add_trace(x = ~ cancer_type, y = as.formula(paste0("~`", g, "`")),
                       name = g, 
                       text = paste("Somatic mutation rate: ", round(mutRates[[g]], digits = 2) , "%")
                       #color = mutRates[[g]]
                      )
}
...
for (c in names(mafs)){
  rbd <- data.frame(gene = rownames(mutsig_p), p = mutsig_p[[c]])
  svg <- svglite::xmlSVG(
         code = { try(PlotOncogenicPathways(maf = mafs[[c]], pathways = "PI3K")
                  title(main = c)},
         standalone = T)
  p[[c]] <- svg
}
